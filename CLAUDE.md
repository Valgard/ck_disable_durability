...
`scripts/link.sh` is a one-time setup step that symlinks `src/`, `src/Editor/`, and `config/` into `$SDK_PATH/Assets/DisableDurability/`. Re-run after moving either repo.
