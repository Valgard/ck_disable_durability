...
- **`DisableDurabilityMod` (`IMod`)** — bootstrap. `Init()` does two things: `BurstDisabler.DisableBurstForSystem<ChangeDurabilitySystem>()` (Burst-compiled `OnUpdate` methods are not patchable by Harmony; this moves the system out of Burst so the Prefix can intercept), and it declares the Mod Settings Menu section — `ModSettings.Section(this).Toggle(out var en, "enabled", true).Build()` — then hands the resulting handle to `ModConfig.Instance.Bind(en)`.
